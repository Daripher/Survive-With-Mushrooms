...
public class MushroomBlock extends BushBlock
{
	...
	{
		super(BlockBehaviour.Properties.of(Material.PLANT).lightLevel(blockState -> 10).noCollission().randomTicks().instabreak().sound(SoundType.GRASS)
	...
	public void randomTick(BlockState blockState, ServerLevel level, BlockPos blockPos, Random random)
	{
		if (random.nextInt(25) == 0)
		{
			...
			for (BlockPos pos : BlockPos.betweenClosed(blockPos.offset(-4, -1, -4), blockPos.offset(4, 1, 4)))
			{
			...
			BlockPos pos = blockPos.offset(random.nextInt(3) - 1, random.nextInt(2) - random.nextInt(2), random.nextInt(3) - 1);
